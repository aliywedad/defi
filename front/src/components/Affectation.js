...
useEffect(() => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/list_soumission/');
      if (!response.ok) {
...
if(render==='add')return <Add setrender={setrender} id_sou={id} />
 else
return(  
      <div className="container-xxl flex-grow-1 container-p-y">
      <h4 className="py-3 mb-4"><span className="text-muted fw-light" > List des soumissions</span></h4>
                    ...
                    <tbody className="table-border-bottom-0">
                       {donnerr.map(item=>(
                        <tr>
                            ...
                            <td className='p-4'> 
                            <a   onClick={()=>{setrender("add");setid(item.id)}} className="btn btn-primary m-2" > Affecter </a>
                             </td>
                        </tr>
                       ))}
                    </tbody>
